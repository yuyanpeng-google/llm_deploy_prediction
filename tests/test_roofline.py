...
class TestRooflineCalculations(unittest.TestCase):
    ...
    def test_calculate_communication_latency_moe_ep_with_attn_tp_all_gather(self) -> None:
        '''Test communication latency for moe_ep with attn_tp and all_gather (attn_dp=1).'''
        config = get_default_config(
            hidden_size=1024,
            num_activated_experts=2,
            bytes_per_param=2
        ...
        # attn_tp=4, attn_dp=1, moe_tp=1, moe_ep=4
        strategy = ShardingStrategy(num_chips=4, attn_tp_degree=4, attn_dp_degree=1, moe_tp_degree=1, moe_ep_degree=4, moe_comm_type='all_gather')
        seq_len = 128
        batch_size = 1
        latency: float = calculate_communication_latency(config, strategy, hardware, seq_len, batch_size, is_prefill=True)
        ...
        # 2. MoE All-Reduce (from moe_ep=4)
        ep = strategy.moe_ep_degree
        data_size_moe = batch_size * seq_len * config.hidden_size * config.bytes_per_param
        latency_ar = 2 * ((ep - 1) / ep) * data_size_moe / 1e9 / bw_ar
